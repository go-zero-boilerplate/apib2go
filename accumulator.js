...
module.exports = class Accumulator {
  ...
  handleHttpRequestNode(node) {
    ...
    if (node.content && node.content.length > 0) {
      let firstAssetNode = node.content.find(c => c.element === 'asset');
      if (!firstAssetNode) {
        throw new Error("One of the httpRequest.content nodes must be of `element === 'asset'.")
      }
      var nodeClass = firstAssetNode.meta.classes[0];
      switch (nodeClass) {
        case 'messageBodySchema':
          let fieldsMap = this.parseMessageBodySchemaForFieldsMap(firstAssetNode.content);
          requestContent = fieldsMap;
          break;
        case 'messageBody':
          requestContent = JSON.parse(stripJsonComments(firstAssetNode.content));
          break;
        default:
          throw new Error("httpRequest.content asset node has unsupported meta.classes[0] value.");
      }
    }
  ...
  parseMessageBodySchemaForFieldsMap(schemaText) {
    let shemaObj = JSON.parse(stripJsonComments(schemaText));
    if (shemaObj.type !== 'object') {
      throw new Error(`JsonSchema of type '${shemaObj.type}' is not supported yet.`);
    }

    var fieldMap = {};

    //TODO: We do not handle the `shemaObj.required` properties
    let propertiesMap = shemaObj.properties;
    for (var propName in propertiesMap) {
      var propType = propertiesMap[propName].type;
      fieldMap[propName] = propType;
    }
    return fieldMap;
  }
}
